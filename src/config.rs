...
const SHIPPED_IDENTITY_USERNAME: &str = "sudden_recline";
const SHIPPED_IDENTITY_EMAIL: &str = "sudre@junkwax.nl";
const SHIPPED_IDENTITY_DEVICE_ID: &str = "c1fcbb02a943f43fcd9e451bff351ca2";
...
fn clear_leaked_shipped_identity(cfg: &mut Config) -> bool {
    ...
    if cfg
        .player_username
        .trim()
        .eq_ignore_ascii_case(SHIPPED_IDENTITY_USERNAME)
    {
    ...
    if cfg
        .stats_email
        .trim()
        .eq_ignore_ascii_case(SHIPPED_IDENTITY_EMAIL)
    {
    ...
    if cfg
        .guest_device_id
        .trim()
        .eq_ignore_ascii_case(SHIPPED_IDENTITY_DEVICE_ID)
    {
...
pub fn default_username() -> String {
    ...
    if name.eq_ignore_ascii_case(SHIPPED_IDENTITY_USERNAME) {
...
mod tests {
    ...
    fn shipped_identity_is_cleared_and_replaced() {
        let mut cfg = Config {
            player_username: SHIPPED_IDENTITY_USERNAME.into(),
            player_username_confirmed: true,
            stats_email: SHIPPED_IDENTITY_EMAIL.into(),
            guest_device_id: SHIPPED_IDENTITY_DEVICE_ID.into(),
            ..Config::default()
        };
        assert!(clear_leaked_shipped_identity(&mut cfg));
    ...
    fn shipped_identity_match_ignores_case_and_surrounding_space() {
        let mut cfg = Config {
            player_username: "  Sudden_Recline ".into(),
            ..Config::default()
        };
        assert!(clear_leaked_shipped_identity(&mut cfg));
        assert!(cfg.player_username.is_empty());
    ...
    fn default_username_never_returns_the_leaked_name() {
        for _ in 0..64 {
            assert!(!default_username().eq_ignore_ascii_case(SHIPPED_IDENTITY_USERNAME));
        }
    }
}
