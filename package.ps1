...
if (Test-Path "config.toml") {
    ...
    if (Test-Path $cfgPath) {
        ...
        # Drop per-install identity. This config is the packager's own, so
        # shipping it verbatim handed every download the same display name
        # (marked confirmed, so the app never offered to pick another), the
        # same stats email, and the same guest_device_id — the value that is
        # supposed to be unique per install and anchors a player's guest
        # stats profile server-side. Removing the keys entirely (rather than
        # blanking them) lets config::load() generate fresh values on first
        # run. Settings that are genuinely install-agnostic — video, audio,
        # bindings, URLs — stay, since shipping good defaults is the point of
        # bundling a config at all.
        $identityKeys = @(
            'player_username',
            'player_username_confirmed',
            'player_username_autogenerated',
            'stats_email',
            'guest_device_id'
        )
        foreach ($key in $identityKeys) {
            $cfg = $cfg -replace "(?m)^\s*$key\s*=.*\r?\n?", ''
        }
        ...
        Write-Host "  ✓ Scrubbed webhook URL and per-install identity from config.toml"
    }
}
